...
C     Continue only if IMODE is 0, 4 or 5
      ...
      IF (ABS(LPP(IB(1))).GE.1) THEN
        ...
        SX1=PDG2PDF(ABS(LPP(IB(1))),-3*LP,XBK(IB(1)),DSQRT(Q2FACT(1)))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u~ u~ > b1 b1~ u~ u~
      PD(IPROC)=UX1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c~ c~ > b1 b1~ c~ c~
      PD(IPROC)=CX1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d~ d~ > b1 b1~ d~ d~
      PD(IPROC)=DX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ s~ > b1 b1~ s~ s~
      PD(IPROC)=SX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG3 = PD(0)
