...
C     Continue only if IMODE is 0, 4 or 5
      ...
      IF (ABS(LPP(IB(2))).GE.1) THEN
        ...
        U2=PDG2PDF(ABS(LPP(IB(2))),2*LP,XBK(IB(2)),DSQRT(Q2FACT(2)))
        S2=PDG2PDF(ABS(LPP(IB(2))),3*LP,XBK(IB(2)),DSQRT(Q2FACT(2)))
        C2=PDG2PDF(ABS(LPP(IB(2))),4*LP,XBK(IB(2)),DSQRT(Q2FACT(2)))
      ENDIF
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g u > b1 b1~ g u
      PD(IPROC)=G1*U2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g c > b1 b1~ g c
      PD(IPROC)=G1*C2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g d > b1 b1~ g d
      PD(IPROC)=G1*D2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g s > b1 b1~ g s
      PD(IPROC)=G1*S2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
