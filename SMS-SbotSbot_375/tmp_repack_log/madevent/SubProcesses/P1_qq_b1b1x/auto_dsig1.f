...
C     
C     Process: u u~ > b1 b1~ WEIGHTED<=2 @1
C     Process: c c~ > b1 b1~ WEIGHTED<=2 @1
C     Process: d d~ > b1 b1~ WEIGHTED<=2 @1
C     Process: s s~ > b1 b1~ WEIGHTED<=2 @1
C     
...
C     Continue only if IMODE is 0, 4 or 5
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u u~ > b1 b1~
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c~ > b1 b1~
      PD(IPROC)=C1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d d~ > b1 b1~
      PD(IPROC)=D1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s s~ > b1 b1~
      PD(IPROC)=S1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG1 = PD(0)
