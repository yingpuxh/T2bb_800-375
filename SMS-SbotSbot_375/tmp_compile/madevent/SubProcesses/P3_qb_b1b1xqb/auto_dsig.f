...
C       Output weights and number of events
        ...
        DO J=1,SYMCONF(0)
          WRITE(*,'(32E12.4)')((SUMWGT(K,I,J)/SUMPROB,K=1,2),I=1
     $     ,MAXSPROC)
        ENDDO
        SUMPROB=0D0
        DO J=1,SYMCONF(0)
        ...
        DO J=1,SYMCONF(0)
          WRITE(*,'(32E12.4)')((NUMEVTS(K,I,J)/SUMPROB,K=1,2),I=1
     $     ,MAXSPROC)
        ENDDO
        WRITE(*,*)'Events:'
        DO J=1,SYMCONF(0)
          WRITE(*,'(32I12)')((NUMEVTS(K,I,J),K=1,2),I=1,MAXSPROC)
        ENDDO
...
C     
     ...
     $ ,DSIG9,DSIG10,DSIG11,DSIG12,DSIG13,DSIG14,DSIG15,DSIG16
      LOGICAL PASSCUTS
C     
C     LOCAL VARIABLES 
C     
...
C       Flip CM_RAP (to get rapidity right)
      ...
      IF (PASSCUTS(P1)) THEN
        IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! u b > b1 b1~ u b
        IF(IPROC.EQ.2) DSIGPROC=DSIG2(P1,WGT,IMODE)  ! u b~ > b1 b1~ u b~
        IF(IPROC.EQ.3) DSIGPROC=DSIG3(P1,WGT,IMODE)  ! c b > b1 b1~ c b
        IF(IPROC.EQ.4) DSIGPROC=DSIG4(P1,WGT,IMODE)  ! c b~ > b1 b1~ c b~
        IF(IPROC.EQ.5) DSIGPROC=DSIG5(P1,WGT,IMODE)  ! d b > b1 b1~ d b
        IF(IPROC.EQ.6) DSIGPROC=DSIG6(P1,WGT,IMODE)  ! d b~ > b1 b1~ d b~
        IF(IPROC.EQ.7) DSIGPROC=DSIG7(P1,WGT,IMODE)  ! s b > b1 b1~ s b
        IF(IPROC.EQ.8) DSIGPROC=DSIG8(P1,WGT,IMODE)  ! s b~ > b1 b1~ s b~
        IF(IPROC.EQ.9) DSIGPROC=DSIG9(P1,WGT,IMODE)  ! u~ b > b1 b1~ u~ b
        IF(IPROC.EQ.10) DSIGPROC=DSIG10(P1,WGT,IMODE)  ! u~ b~ > b1 b1~ u~ b~
        IF(IPROC.EQ.11) DSIGPROC=DSIG11(P1,WGT,IMODE)  ! c~ b > b1 b1~ c~ b
        IF(IPROC.EQ.12) DSIGPROC=DSIG12(P1,WGT,IMODE)  ! c~ b~ > b1 b1~ c~ b~
        IF(IPROC.EQ.13) DSIGPROC=DSIG13(P1,WGT,IMODE)  ! d~ b > b1 b1~ d~ b
        IF(IPROC.EQ.14) DSIGPROC=DSIG14(P1,WGT,IMODE)  ! d~ b~ > b1 b1~ d~ b~
        IF(IPROC.EQ.15) DSIGPROC=DSIG15(P1,WGT,IMODE)  ! s~ b > b1 b1~ s~ b
        IF(IPROC.EQ.16) DSIGPROC=DSIG16(P1,WGT,IMODE)  ! s~ b~ > b1 b1~ s~ b~
      ENDIF
