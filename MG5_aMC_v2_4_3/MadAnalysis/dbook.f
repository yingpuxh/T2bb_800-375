...
C
	 ...
	 CALL MWARN('MBOOK')
      ...
      TITLE(N) = ' '//TIT
1     HDEL(N) = DEL
      NBIN(N) = INT((XMAX-XMIN)/DEL+0.00001d0)
	...
	GO TO 1
      ...
      IF(abs(HMAX(N)-XMAX).gt.0.001d0) THEN
	 CALL MWARN('MBOOK')
         WRITE(*,*)
...
c RF
c      SUBROUTINE MTOP(N,M,BTIT,LTIT,SCALE)
      SUBROUTINE MTOP(N,M,BTIT,LTIT,SCALE,nevent,sum,norm,gnu)
cend
...
C
      ...
      if (gnu) then
         call mtopgnuplot(N,M,BTIT,LTIT,SCALE,nevent,sum,norm)
      else
         call MTOPtdrawer(N,M,BTIT,LTIT,SCALE,nevent,sum,norm)
      endif


      END

      SUBROUTINE MTOPtdrawer(N,M,BTIT,LTIT,SCALE,nevent,sum,norm)
...
c         IF(abs(HIST(N,J)).le.maxbin*10d-7) GO TO 1
         IF(abs(HIST(N,J  )).le.maxbin*10d-7) THEN
            IF(J.EQ.1.)THEN
               IF(abs(HIST(N,J+1)).le.maxbin*10d-7) GOTO 1
            ELSE IF(J.EQ.NBIN(N)) THEN
               IF(abs(HIST(N,J-1)).le.maxbin*10d-7) GOTO 1
            ELSE IF(abs(HIST(N,J-1)).le.maxbin*10d-7.and.
     &              abs(HIST(N,J+1)).le.maxbin*10d-7     ) THEN
               GO TO 1
            ENDIF
         ENDIF
         WRITE(99,'(3X,F10.4,2(2X,E15.4))') XHIS(N,J),HIST(N,J)*norm
    1 CONTINUE
      WRITE(99,200)
  200 FORMAT('   HISTO')
      WRITE(99,300) sum,HAVG(N),HSIG(N),nevent,ient(n),IUSCORE(N)
     &   ,IOSCORE(N)
  300 FORMAT( /1x,
     &' (INFO-BOX'/,1X,                               
     ...
     &             '   RMS =',1PE10.3,'"',/1X,
     &' TITLE 3.8 1.1 "Tot # Evts =',I8,2x,'Entries =',I8,2X
  ...
  400 FORMAT('   NEW PLOT')
      ...
      SUBROUTINE MTOPgnuplot(N,M,BTIT,LTIT,SCALE,nevent,sum,norm)
...
c      CALL TIME(CTIME)
  ...
  100 FORMAT( /1x,                               
     ...
     &'# SET ORDER X Y DY ')
      ...
      DO 1 J=1,NBIN(N)
c         IF(abs(HIST(N,J)).le.maxbin*10d-7) GO TO 1
         IF(abs(HIST(N,J  )).le.maxbin*10d-7.and.
     &      abs(HIST(N,J-1)).le.maxbin*10d-7.and.
     &      abs(HIST(N,J+1)).le.maxbin*10d-7     ) GO TO 1
         WRITE(99,'(3X,F10.4,(2X,E15.4))')  
     &                            XHIS(N,J),HIST(N,J)*norm
    1 CONTINUE
      WRITE(99,200)
  200 FORMAT('#   HISTO')
      WRITE(99,300) sum,HAVG(N),HSIG(N),nevent,ient(n),IUSCORE(N)
