...
c
         ...
         if (iter .le. itmax) then
            ...
            if (pass_point(p)) then
               ...
               if ((wgt - 1) .eq. wgt .and. fx .eq. 0) then
c                 (wgt - 1) .eq. wgt tests for inifinity
c                 When wgt is infinity and fx is 0, wgt*fx becomes nan,
c                 which makse the cross cross section nan.
c                 To prevent that, we set wgt = 0 here
                  wgt = 0d0
               else
                  wgt = wgt*fx
               endif
